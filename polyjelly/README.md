...
* [`src/clj/polyjelly/`](src/clj/polyjelly/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/polyjelly/`](src/cljs/polyjelly/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/polyjelly/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `polyjelly` project root directory
...
in the [`src/clj/polyjelly/`](src/clj/polyjelly/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/polyjelly/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns polyjelly.example
  (:require [polyjelly.config :as config])
